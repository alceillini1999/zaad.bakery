...
async function loadExpenses() {
  ...
  if ($('#expNote').value)   p.set('note', $('#expNote').value);
...
$('#btnExpensesFilter')?.addEventListener('click', loadExpenses);
$('#formExpense')?.addEventListener('submit', async e => {
  e.preventDefault();
  const form = e.target;
  const formData = new FormData(form); // sends receipt file if provided
...
async function loadCredit() {
  ...
  let list = rows;
  const customerFilter = ($('#crCustomerFilter').value || '').toLowerCase();
  if (customerFilter) {
    list = list.filter(r => (r.customer || '').toLowerCase().includes(customerFilter));
  }
  const statusFilter = $('#crStatusFilter').value;
  if (statusFilter) {
    list = list.filter(r => {
      const totalPaid = (+r.paid || 0) + (paysBy[(r.customer || '').trim()] || 0);
      const fullyPaid = totalPaid >= (+r.amount || 0);
      return statusFilter === 'paid' ? fullyPaid : !fullyPaid;
    });
  }
...
$('#crCustomerFilter')?.addEventListener('input', loadCredit);
$('#crStatusFilter')?.addEventListener('change', loadCredit);
...
$('#formPay')?.addEventListener('submit', async e => {
  ...
  if (res.ok) {
    ...
    loadCredit();
    loadSales();
    document.querySelector('[data-bs-target="#tabSales"]')?.click();
  } else {
    showToast(res.error || 'Failed', false);
  }
});
...
async function loadOrders() {
  ...
  let list = rows;
  const customerFilter = ($('#orCustomerFilter').value || '').toLowerCase();
  if (customerFilter) {
    list = list.filter(r => (r.phone || '').toLowerCase().includes(customerFilter));
  }
...
$('#btnOrdersFilter')?.addEventListener('click', loadOrders);
...
$('#formOrderPay')?.addEventListener('submit', async e => {
  e.preventDefault();
  const id = $('#opId').value;
  const remain = parseFloat($('#opRemaining').value || '0');
  const amount = parseFloat($('#opAmount').value || '0');
  if (amount > remain) {
    return showToast('Amount exceeds remaining', false);
  }
...
function updateCashTotal() {
  ...
  $$('input[id^="d_"]').forEach(i => {
    const denom = parseInt(i.id.split('_')[1], 10);
    const qty = parseInt(i.value || '0', 10);
    total += denom * qty;
  });
...
function drawSalesByMethod({cash, till, withdrawal, send}) {
  const ctx = document.getElementById('salesByMethodChart');
  if (!ctx) return;
  ...
  salesByMethodChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: ['Cash','Till No','Withdrawal','Send Money'],
      datasets: [{ label: 'Sales (KES)', data }]
    },
    options: {
      responsive: true,
...
async function runReport() {
  ...
  // Section 4: Cash available (cash morning + cash sales + withdrawal-out - cash expenses)
  ...
  // Manual cash_out for the selected day (session='cash_out')
  let manualCashOut = 0;
  if (from === to) {
  ...
  // Build sections for report cards
  const sections = [
  ...
  // Prefill & Save manual Cash Out UI (report Till No Out field)
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadDenoms();
  if (!ENABLE_CHART) {
    const chartCanvas = $('#salesByMethodChart');
...
// (Reports) Till No Out manual box handlers
(function() {
  ...
  if (from !== to) {
    ...
    input.disabled = true;
    btn.disabled = true;
    input.placeholder = 'اختر يوم واحد';
    return;
  }
  // Load existing till_out (if any)
...
$('#btnCashOutSave')?.addEventListener('click', async () => {
  const val = +($('#cashOutInput').value || 0);
  if (!(val >= 0)) return showToast('أدخل رقم صالح', false);
...
function toggleCashMode() {
  ...
  if (s === 'till_out' || s === 'send_out') {
    denoms.classList.add('d-none');
    man.classList.remove('d-none');
    updateCashTotalManual();
  } else {
    man.classList.add('d-none');
    denoms.classList.remove('d-none');
    updateCashTotal();
  }
}
// listeners for Cash tab mode
document.addEventListener('DOMContentLoaded', () => {
