// server.js
// Zaad Bakery — JSON storage + Realtime + Filters + CSV/XLSX/PDF + Credit customers + Basic Auth + Cloud/LAN ready
...
app.post('/api/sales', (req, res) => {
  ...
  // de-dupe (لو اتسجّل نفس الصف خلال 1.5 ثانية)
  const last = arr[arr.length - 1];
  const tooClose    = last && Math.abs(new Date(nowIso) - new Date(last.date)) < 1500;
  const samePayload = last && last.amount === row.amount &&
...
app.post('/api/expenses', (req, res) => {
  ...
  const arr = readJSON(fp);
  arr.push({ amount:+amount, category, method: method || 'Cash', note, date: new Date().toISOString() });
  writeJSON(fp, arr);
  res.json({ ok: true });
});
...
app.post('/api/cash', (req, res) => {
  ...
  const d = toDay(date);
  const fp = p('cash', `${d}.json`);
  const arr = readJSON(fp);
  const den = denominations || {};
  let total = 0; Object.keys(den).forEach(k => total += (+k) * (+den[k] || 0));
  arr.push({ shift: shift || 'morning', denominations: den, total, note, date: new Date().toISOString() });
  writeJSON(fp, arr);
...
app.post('/api/credit/purchase', (req, res) => {
  ...
  arr.push({ type:'purchase', amount:+amount, note, date: date || new Date().toISOString() });
  writeJSON(creditFile(customer), arr);
  res.json({ ok: true });
});
app.post('/api/credit/payment', (req, res) => {
  ...
  if (!customer || amount == null) return res.status(400).send('customer & amount required');
  const arr = readJSON(creditFile(customer));
  arr.push({ type:'payment', amount:+amount, method: method || 'Cash', note, date: date || new Date().toISOString() });
  writeJSON(creditFile(customer), arr);
  res.json({ ok: true });
});
...
app.get('/api/reports/daily', (req, res) => {
  ...
  const cash = readJSON(p('cash', `${d}.json`));
...
app.get('/api/export/csv/cash', (req, res) => {
  const d = toDay(req.query.date);
  const rows = readJSON(p('cash', `${d}.json`)).map(r => ({ date:r.date, shift:r.shift, total:r.total, note:r.note||'', denominations: JSON.stringify(r.denominations||{}) }));
...
app.get('/api/export/pdf/daily', async (req, res) => {
  ...
  doc.pipe(stream);
  doc.fontSize(20).text('Zaad Bakery — Daily Report', { align: 'center' });
  doc.moveDown();
